...
correct=$(aoc --session-file aoc-session.txt read -y "$year" -d "$day" 2>/dev/null | grep "Your puzzle answer was" | sed -n "${part}p" | sed 's/.*Your puzzle answer was \(.*\)\..*/\1/' || true)
...
response=$(aoc --session-file aoc-session.txt submit -y "$year" -d "$day" "$part" "$answer" 2>&1)
